...
dd if=$1 of=before-bbEntry-dex.tmp bs=1 count=$bbEntryPos
dd if=$1 of=after-bbEntry-dex.tmp bs=1 skip=$bbEntryPosAfter
echo $op | xxd -r > opcode.tmp
echo $alignmentHex | xxd -r >> opcode.tmp
#echo 0x50 | xxd -r >> opcode.tmp
dd if=/dev/zero of=padding.tmp bs=1 count=$left

cat before-bbEntry-dex.tmp opcode.tmp after-bbEntry-dex.tmp padding.tmp $2 > new-classes.dex.tmp
